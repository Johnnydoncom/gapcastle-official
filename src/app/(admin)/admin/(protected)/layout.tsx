...
export default async function ProtectedAdminLayout({ children }: LayoutProps<"/admin">) {
  ...
  return (
    <div className="min-h-full flex-1 lg:pl-[17.5rem]">
      <AdminShellNav user={{ name: user.name, email: user.email, role: user.role }} counts={counts} />      <main className="mx-auto w-full max-w-[84rem] px-4 py-6 sm:px-6 sm:py-8 lg:px-10 lg:py-10">{children}</main>
    </div>
  );
}
